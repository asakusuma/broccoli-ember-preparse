...
var processLoader = require('./process-loader');
...
DefineRewriter.prototype.processString = function(string, relativePath) {
  if (EMBER_PATHS[relativePath]) {
    return processEmber(string, relativePath, this.eagerLoad, this.loaderDeferred.promise);
  } else if (relativePath.indexOf('loader') > -1) {
    // If it's the loader file, read and save in deferred for use by preparse
    this.loaderDeferred.resolve(processLoader(string, relativePath));
  }
  return string;
};
